...
PrepareResult prepare_statement(InputBuffer* input_buffer,Statement* statement ){
  if (strncmp(input_buffer->buffer, "insert", 6) ==0){
    ...
    int args_assigned = sscanf(  //sscanf函数返回被读取和保存的插入字段
        input_buffer->buffer, "insert %d %s %s", //buffer赋予''语句中的占位符
        &(statement->row_to_insert.id),          //插入行int取地址使用&
        statement->row_to_insert.username,       //插入行字符串，使用的是数组创建故username为地址指针
        statement->row_to_insert.email);         //同上
    if(args_assigned < 3) {
